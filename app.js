...
async function signOutUser() {
  if (firebaseReady && firebaseAuth && firebaseModules) {
    await firebaseModules.signOut(firebaseAuth).catch((error) => console.warn("Sign-out failed.", error));
  }
  currentUser = null;
  localMode = false;
  updateAuthUI();
...
function asItem(entry) {
  if (entry && entry.exam && entry.question) return entry;
  return { exam: state.exam, question: entry };
}

function questionKey(entry) {
  const item = asItem(entry);
  return `${quizIdForExam(item.exam)}_${item.question.no}`;
}

function savedAnswer(entry) {
  const item = asItem(entry);
  return loadLocalProgress(item.exam)?.answers?.[item.question.no] || "";
}
...
function scoreForItems(items, tempAnswers = null) {
  let good = 0;
  let bad = 0;
  for (const rawItem of items) {
    const item = asItem(rawItem);
    const picked = tempAnswers ? tempAnswers[questionKey(item)] : savedAnswer(item);
    if (!picked) continue;
    if (isCorrect(picked, item.question.answer)) good += 1;
    else bad += 1;
  }
  return { good, bad, done: good + bad, total: items.length };
}

function scoreForQuestions(exam, questions, answers = null) {
  const items = questions.map((question) => ({ exam, question }));
  if (answers) return scoreForItems(items, answers);
  return scoreForItems(items);
}

function score(exam) {
  return scoreForQuestions(exam, exam.questions);
}

function subjectQuestions(exam, subjectNo) {
  if (!subjectNo) return exam.questions;
  const start = (subjectNo - 1) * 20 + 1;
  const end = start + 19;
  return exam.questions.filter((question) => question.no >= start && question.no <= end);
}

function subjectQuestionItems(exam, subjectNo) {
  return subjectQuestions(exam, subjectNo).map((question) => ({ exam, question }));
}

function currentBaseQuestions() {
  if (state.multiExamMode) return state.multiQuestions || [];
  return subjectQuestionItems(state.exam, state.subjectNo);
}

function currentQuestions() {
  return state.wrongMode && state.wrongList ? state.wrongList : currentBaseQuestions();
}

function currentQuestionItem() {
  return asItem(currentQuestions()[state.idx]);
}

function currentQuestionNo() {
  return currentQuestionItem()?.question?.no || state.exam?.questions?.[0]?.no || 1;
}

function currentScopeLabel() {
  if (state.multiExamMode) return state.multiLabel || "전년도 오답만 다시 풀기";
  if (state.subjectNo && state.wrongMode) return `${state.subjectNo}과목 오답만 다시 풀기`;
  if (state.subjectNo) return `${state.subjectNo}과목 전체 풀기`;
  if (state.wrongMode) return "전체 오답만 다시 풀기";
  return "전체 문제 풀기";
}

function previousWrongItems(referenceYear = null) {
  const yearList = years();
  const baseYear = referenceYear || yearList[0];
  return QUIZ_DATA
    .filter((exam) => exam.year < baseYear)
    .sort((a, b) => b.year - a.year || b.round - a.round)
    .flatMap((exam) => {
      const record = loadLocalProgress(exam)?.answers || {};
      return exam.questions
        .filter((question) => record[question.no] && !isCorrect(record[question.no], question.answer))
        .map((question) => ({ exam, question }));
    });
}

function previousWrongLabel(referenceYear = null) {
  const baseYear = referenceYear || years()[0];
  return `${baseYear}년 이전 오답만 다시 풀기`;
}

function persistCurrentProgress(options = {}) {
  if (!state.exam || state.multiExamMode) return null;
  ...
  if (currentUser && firebaseReady) {
    ...
    if (options.immediate) flushCloudProgress(progress.quizId, progress);
    else scheduleCloudSave(progress);
  } else {
    setSyncStatus("로컬 저장됨", "local");
  }
...
function scheduleCloudSave(progress) {
  ...
  if (saveTimers.has(progress.quizId)) clearTimeout(saveTimers.get(progress.quizId));
  saveTimers.set(progress.quizId, setTimeout(() => flushCloudProgress(progress.quizId), 800));
}
...
function renderPreviousWrongHomeButton() {
  const card = $("#homeView .card");
  if (!card) return;
  let panel = $("#previousWrongPanel");
  if (!panel) {
    panel = document.createElement("div");
    panel.id = "previousWrongPanel";
    panel.style.marginTop = "12px";
    card.appendChild(panel);
  }
  const count = previousWrongItems().length;
  const baseYear = years()[0];
  panel.innerHTML = `<div class="actions"><button class="btn good" id="previousWrongHomeBtn">전년도 오답 풀기 (${count})</button></div><p class="small">${baseYear}년 이전 회차에서 틀린 문제만 모아서 다시 풉니다.</p>`;
  $("#previousWrongHomeBtn").onclick = () => startPreviousWrong();
}
...
function renderPreviousWrongRoundButton(year) {
  const actions = $("#roundView .card .actions");
  if (!actions) return;
  let button = $("#previousWrongRoundBtn");
  if (!button) {
    button = document.createElement("button");
    button.id = "previousWrongRoundBtn";
    button.className = "btn good";
    actions.appendChild(button);
  }
  const count = previousWrongItems(year).length;
  button.textContent = `${year}년 이전 오답 풀기 (${count})`;
  button.onclick = () => startPreviousWrong(year);
}
...
function renderRounds(year) {
  ...
  renderPreviousWrongRoundButton(year);
...
function startExam(exam, idx = null) {
  ...
  state.multiExamMode = false;
  state.multiQuestions = null;
  state.multiLabel = "";
  renderQuiz();
}

function startScope(subjectNo, wrongOnly = false) {
  const base = subjectQuestionItems(state.exam, subjectNo);
  state.subjectNo = subjectNo;
  state.wrongMode = wrongOnly;
  state.wrongList = wrongOnly
    ? base.filter((item) => savedAnswer(item) && !isCorrect(savedAnswer(item), item.question.answer))
    : null;
  state.wrongPracticeAnswers = {};
  state.multiExamMode = false;
  state.multiQuestions = null;
...
function startPreviousWrong(referenceYear = null) {
  const items = previousWrongItems(referenceYear);
  if (!items.length) {
    toast("전년도 오답이 없어. 오답 창고가 텅 비었습니다");
    return;
  }
  state.exam = items[0].exam;
  state.idx = 0;
  state.subjectNo = null;
  state.wrongMode = true;
  state.wrongList = items;
  state.wrongPracticeAnswers = {};
  state.multiExamMode = true;
  state.multiQuestions = items;
  state.multiLabel = previousWrongLabel(referenceYear);
  renderQuiz();
}
...
function renderSubjectPanel() {
  ...
  if (!panel) {
    ...
    panel.style.marginTop = "12px";
    card.insertBefore(panel, $("#examStats"));
  }

  if (state.multiExamMode) {
    panel.innerHTML = `<p class="small" style="margin:10px 0 6px"><b>전년도 오답 풀기</b> · 여러 연도/회차의 오답을 한 번에 다시 푸는 중입니다.</p>`;
    return;
  }

  const buttons = [1, 2, 3, 4, 5]
    .map((subjectNo) => `<div class="actions" style="margin-top:7px"><button class="btn secondary" data-subject-mode="all" data-subject="${subjectNo}">${subjectNo}과목 전체풀기</button><button class="btn good" data-subject-mode="wrong" data-subject="${subjectNo}">${subjectNo}과목 오답만풀기</button></div>`)
    .join("");

  panel.innerHTML = `<p class="small" style="margin:10px 0 6px"><b>과목별 풀기</b> · 과목별 전체/오답만 따로 연습할 수 있습니다.</p>${buttons}`;
  $$('[data-subject-mode]').forEach((button) => {
    button.onclick = () => startScope(Number(button.dataset.subject), button.dataset.subjectMode === "wrong");
  });
}
...
function renderQuiz() {
  ...
  const picked = state.wrongMode ? state.wrongPracticeAnswers[questionKey(item)] : savedAnswer(item);

  $("#appTitle").textContent = `${exam.year}년 ${exam.round}회`;
  $("#appSub").textContent = currentScopeLabel();
  $("#qNo").textContent = state.multiExamMode ? `${exam.year}년 ${exam.round}회 ${question.no}번` : `${question.no}번`;
...
function choose(choice) {
  ...
  if (state.wrongMode) state.wrongPracticeAnswers[questionKey(item)] = choice;
...
function renderExamStats() {
  const answers = state.wrongMode ? state.wrongPracticeAnswers : null;
  const stats = scoreForItems(currentQuestions(), answers);
  $("#examStats").innerHTML = `<div class="stat"><b>${stats.done}/${stats.total}</b><span>${currentScopeLabel()}</span></div><div class="stat"><b>${stats.good}</b><span>정답</span></div><div class="stat"><b>${stats.bad}</b><span>오답</span></div>`;
}
...
function renderList() {
  setView("list");
  const questions = currentQuestions();
  $("#appTitle").textContent = state.multiExamMode ? "전년도 오답 문항표" : `${state.exam.year}년 ${state.exam.round}회 문항표`;
  $("#appSub").textContent = currentScopeLabel();
  $("#numList").innerHTML = questions
    .map((rawItem, index) => {
      const item = asItem(rawItem);
      const picked = state.wrongMode ? state.wrongPracticeAnswers[questionKey(item)] : savedAnswer(item);
      let className = "";
      if (picked) className = isCorrect(picked, item.question.answer) ? "good" : "bad";
      const label = state.multiExamMode ? `${item.exam.year}-${item.exam.round}/${item.question.no}` : item.question.no;
      return `<button class="numBtn ${className}" data-index="${index}">${label}</button>`;
    })
    .join("");
...
function finish() {
  if (state.multiExamMode) {
    const stats = scoreForItems(currentQuestions(), state.wrongPracticeAnswers);
    alert(`${currentScopeLabel()} 결과\n\n풀이: ${stats.done}/${stats.total}\n정답: ${stats.good}\n오답: ${stats.bad}\n정답률: ${stats.done ? Math.round((stats.good / stats.done) * 100) : 0}%\n저장: ${syncStatusText()}`);
    return;
  }
...
function bindEvents() {
  ...
  $("#resetExam").onclick = async () => {
    if (state.multiExamMode) {
      toast("전년도 오답 모드에서는 회차 초기화를 사용할 수 없습니다");
      return;
    }
  ...
  $("#resetYear").onclick = async () => {
    ...
    for (const exam of exams) await resetExamProgress(exam);
    renderRounds(state.year);
  };

  $("#goFirstWrong").onclick = () => {
    if (state.multiExamMode) startPreviousWrong(state.year || null);
    else startScope(state.subjectNo, true);
  };
